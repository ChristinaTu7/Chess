...
public class Piece {
   ...
   * If a piece is a pawn, check to see which color it is
   * Returns a boolean if it's in the right row
   */
  ...
  public void promotePawn(String user, Board board, int row, int col, boolean isBlack) {
    Piece promoPiece;
    switch (user) {
      case "queen":
        if (isBlack) {
          promoPiece = new Piece('\u265b', row, col, true);
        } else {
          promoPiece = new Piece('\u2655', row, col, false);
        }
        board.setPiece(row, col, promoPiece);
